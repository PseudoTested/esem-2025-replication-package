...
logger_jar = (
    os.path.dirname(slicer4j_dir)
    + "/DynamicSlicingCore/DynamicSlicingLoggingClasses/DynamicSlicingLogger.jar"
)
...
def main():
    ...
    if (not main_class_args) and (not test_class or not test_method):
        print(
            "Must provide either main class name and arguments or test class and test method"
        )
        return
    if framework_models:
    ...
    log_file, slice_graph = dynamic_slice(
        jar_file=jar_file,
        out_dir=out_dir,
        backward_criterion=backward_criterion,
...
def run(
    ...
    if main_class_args is None:
        print("JUnit Version", type(junit_version))
        if junit_version == "5":
            cmd = f'java -Xmx16g  -cp "resources/SingleJUnit5TestRunner/target/SingleJUnit5TestRunner-1.0-SNAPSHOT.jar:resources/SingleJUnit5TestRunner/target/dependency/*:{instrumented_jar}:{dependencies}/*" org.pseudosweep.SingleJUnit5TestRunner {test_class}#{test_method} > {out_dir}/trace_full.log'
        else:
...
def dynamic_slice(
    ...
    if variables:
        print(
            f"Slicing from line {backward_criterion} with variables {variables}",
            flush=True,
        )
    else:
...
def parse():
    ...
    parser.add_argument(
        "-b",
        "--backward-criterion",
        dest="backward_criterion",
        help="Backward criterion (line number)",
        metavar="line to slice backward from",
        required=True,
    )
    parser.add_argument(
        "-v",
        "--variables",
        dest="variables",
        help="Variables to slice from, list of - separated names",
        metavar="variables to slice from",
        required=False,
    )
    parser.add_argument(
        "-mod",
        "--models",
        dest="framework_models",
        help="Folder containing user-defined method models",
        metavar="user defined framework models",
        required=False,
    )
    parser.add_argument(
        "-debug",
    ...
    parser.add_argument(
        ...
        "--data",
        dest="data_only",
        help="Slice with data-flow dependencies only",
        action="store_true",
        required=False,
    )
    parser.add_argument(
        ...
        "--control",
        dest="ctrl_only",
        help="Slice with control dependencies only",
        action="store_true",
        required=False,
    )
    parser.add_argument(
        ...
        "--once",
        dest="once",
        help="Slice for first statement only then stop",
        action="store_true",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-m",
        "--main-class-args",
        dest="main_class_args",
        help="Main class to run with arguments",
        metavar="name",
        required=False,
    )
    parser.add_argument(
        ...
        "--dependencies",
        dest="dependencies",
        help="JAR dependencies",
        metavar="path",
        required=False,
    )
    parser.add_argument("-u", dest="junit", help="JUnit Version")
    args = parser.parse_args()
    return {
        "jar_file": args.jar_file,
        "out_dir": args.out_dir,
        "backward_criterion": args.backward_criterion,
        "variables": args.variables,
        "data_only": args.data_only,
        "ctrl_only": args.ctrl_only,
        "test_class": args.test_class,
        "test_method": args.test_method,
        "main_class_args": args.main_class_args,
